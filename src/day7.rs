    ...
    I::Item: AsRef<str>,
{
    ...
    let dir_sizes = all_dirs.iter().map(|d| {
        (d, fs.iter().filter(|(p, _)| p.starts_with(d)).map(|(_, s)| s).sum::<u32>())
    });
...
{
    let (all_dirs, fs) = construct_fs(lines)?;
    let dir_sizes: Vec<(_, _)> = all_dirs.iter().map(|d| {
        (d, fs.iter().filter(|(p, _)| p.starts_with(d)).map(|(_, s)| s).sum::<u32>())
    }).collect();
    ...
    let freeing_dir = dir_sizes.iter().filter(|(_, s)| s >= &&deficit).min_by(|(_, s), (_, s_other)| s.cmp(s_other)).context("finding min dir to free space")?;
    Ok(*freeing_dir.1)
}
...
mod tests {
    ...
    #[test]
    fn test_day7() -> Result<()> {
        assert_eq!(sum_small_dirs(vec!["$ cd /", "$ ls", "dir a", "14848514 b.txt", "8504156 c.dat", "dir d", "$ cd a", "$ ls", "dir e", "29116 f", "2557 g", "62596 h.lst", "$ cd e", "$ ls", "584 i", "$ cd ..", "$ cd ..", "$ cd d", "$ ls", "4060174 j", "8033020 d.log", "5626152 d.ext", "7214296 k"])?, 95437);
        assert_eq!(find_freeing_dir(vec!["$ cd /", "$ ls", "dir a", "14848514 b.txt", "8504156 c.dat", "dir d", "$ cd a", "$ ls", "dir e", "29116 f", "2557 g", "62596 h.lst", "$ cd e", "$ ls", "584 i", "$ cd ..", "$ cd ..", "$ cd d", "$ ls", "4060174 j", "8033020 d.log", "5626152 d.ext", "7214296 k"])?, 24933642);
        Ok(())
    }
}
